...
A retired restored husk leaves its task record, local copy, branch, and steering inbox exactly as they were; the record's endpoint then reads gone, the monitoring reports that once, and a later `bin/fm-control.sh <id> relaunch` reclaims the task by recreating its endpoint flat in the same worktree, which `bin/fm-spawn.sh --relaunch` owns for a positively proven missing Herdr endpoint ([`agent-control.md`](agent-control.md)); a secondmate still refuses, and teardown erases the record on the same confirmed-gone proof it always used.
