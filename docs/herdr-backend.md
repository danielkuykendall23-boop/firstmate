...
A record that names the pane is first compared against the pane's current shell start second from one process-info read, so a parked or exited worker's launch shell is preserved on every cadence without paying for the settle-retry proof, and a live worker is preserved on its agent-state read alone.
...
- Crashes, lost responses, failed exact-pane cleanup, or human renames can leave quarantined spaces; session start and the watcher's slow-check cadence remove only the exact home-local, uniquely journal-correlated, childless idle-shell shape above, whether its task record is gone or names that pane with a server-restored shell.
...
`tests/fm-herdr-session-cleanup-e2e.test.sh` covers the restored-shell cleanup in a guarded non-default named lab, including a metadata-backed server-restored husk, a parked launch-shell pane, a record naming another endpoint, the mutation-free dry run, and a husk that appears mid-session retired by the same housekeeping run after a second server restart; `tests/fm-control-herdr-smoke.test.sh` proves the gone-endpoint relaunch recreate against the real binary.
