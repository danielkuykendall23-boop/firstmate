...
Locked session start has one narrower cleanup for a restored projected child that is no longer usable task state.
...
The candidate must have exactly one tab and exactly one pane, and the task's ordinary metadata must take one of two shapes.
Either it is absent, so the projection outlived its task, or every recorded Herdr identity field names exactly this session, workspace, tab, and pane and the pane's shell started after the metadata's recorded `spawn_gen` launch second, which is the shape a Herdr server restart leaves: the worker process died with the server, and the shell now in the pane is a restored one rather than the one firstmate launched the worker into.
The shell firstmate launched into always predates `spawn_gen`, so a parked or exited worker's pane, which still holds its transcript and is the pane a relaunch adopts, is preserved silently, and a task whose metadata names some other endpoint keeps its projection untouched as well.
Before cleanup, Firstmate acquires the existing task-id spawn lock and then the shared named-session presentation lock, plus the task's record lock when its metadata names the pane so a concurrent relaunch or teardown is excluded.
Inside those locks it takes one exact snapshot, requires one unambiguous non-target focus and the exact title, token, tab, and pane shape, positively confirms no registered agent, and reads Herdr's process information for the exact named-session pane.
...
Firstmate immediately revalidates the same journal, metadata shape and restored-shell proof, workspace title and token uniqueness, one-tab and one-pane topology, exact pane relationship, absent agent, process proof, and non-target focus before calling the existing exact-pane focus-preserving close helper.
...
A retired restored husk leaves its task record, local copy, branch, and steering inbox exactly as they were; the record's endpoint then reads gone, the monitoring reports that once, and a later `bin/fm-control.sh <id> relaunch` recreates the endpoint in the same worktree (projected when no journal remains and the home's presentation gate allows, flat otherwise) because `bin/fm-spawn.sh --relaunch` accepts a structurally gone Herdr pane under a running named session; a stopped session, another backend, or a secondmate still refuses, and teardown erases the record on the same confirmed-gone proof it always used.
A second run finds no matching title or journal and is a no-op.
A malformed or missing title or token, duplicate token, zero or multiple journal matches, cross-home version 2 binding, metadata naming another endpoint or another backend, a launch shell, registered or unknown agent, extra tab or pane, active target, busy lock, changed revalidation, unreadable check, or any error preserves the candidate and lets session startup continue with at most a concise warning.
`bin/fm-herdr-session-cleanup.sh --dry-run` prints the same verdict for every workspace carrying the projection grammar in the named session - `close` or `keep`, with the task and pane identity and the reason - without taking a lock or mutating anything, so an operator can inspect exactly which spaces a locked run would retire; the locked run still re-derives every verdict under its locks, and running it outside session start requires owning this home's session lock exactly as session start does.
...
- Crashes, lost responses, failed exact-pane cleanup, or human renames can leave quarantined spaces; session start removes only the exact home-local, uniquely journal-correlated, childless idle-shell shape above, whether its task record is gone or names that pane with a server-restored shell.
...
`tests/fm-herdr-session-cleanup-e2e.test.sh` covers the restored-shell cleanup in a guarded non-default named lab, including a metadata-backed server-restored husk, a parked launch-shell pane, a record naming another endpoint, and the mutation-free dry run; `tests/fm-control-herdr-smoke.test.sh` proves the gone-endpoint relaunch recreate against the real binary.
