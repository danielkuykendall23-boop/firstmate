...
When the block's only cause is a stale beacon on a live identity-matched lock holder (`fm_watcher_stale_beacon_reason` in `bin/fm-wake-lib.sh`, the shape a system sleep leaves), the guard still blocks but its banner reads `WATCHER BEACON STALE - RECHECK BEFORE REPAIRING` and tells the agent to wait one poll and recheck before repairing; every harness adapter relays that recheck framing instead of its "supervision is off" preamble, which remains for a missing, dead, or foreign lock.
