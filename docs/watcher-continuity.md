...
omp's replacement follows the same generation-owner contract in `.omp/extensions/fm-primary-omp-watch.ts`, whose header owns the omp differences: omp reports no shutdown reason, so every shutdown with a pending actionable close persists the handoff for the next owning activation to replay, and only the primary runner arms because omp hosts every in-process `task` subagent as another runner of the same extension.
...
`tests/fm-omp-harness.test.sh` drives omp's watch extension over a fake omp API: arming, an actionable close delivered as one follow-up, a print-mode subagent runner that stays inert through its start and shutdown, and the primary re-arming after a shutdown through `session_switch` or its own `fm_watch_arm_omp`.
