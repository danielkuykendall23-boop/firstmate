...
2. Confirm the omp primary auto-loaded both required project extensions from `.omp/extensions/`; omp has no project-trust gate, so a plain `omp` started with this home as its working directory loads them with no dialog.
...
6. Ordinary same-process session replacement (`/new`, `/resume`, `/fork`) keeps or replaces the watcher generation without a model turn or another `fm_watch_arm_omp` call; only this primary session arms, and an in-process `task` subagent never arms or receives wakes.
   The generation-owner contract and in-flight actionable-close handoff live in `.omp/extensions/fm-primary-omp-watch.ts`; because omp reports no shutdown reason, every shutdown with a pending actionable close persists the handoff, and the next owning activation in any process replays it.
