...
The handler keeps no memo of a previous decline by design, so this cost stands until omp emits the hook once per compaction.
...
`fm-jev-compaction.test.sh` runs `tests/fm-jev-compaction.node.test.ts` (Node's native TypeScript support, no build step - the same way omp itself loads the extension), covering the omp-message-shape translation, flattening, and excluded-execution omission, the previous-summary merge and its whole-context and retained-budget gates across repeated compactions, the budget derivation against omp's trigger and reserve rules, the decline that keeps a native OpenAI remote or snapcompact compaction's preserved history, the Hide Secrets gate against a stand-in omp binary installed as `process.execPath` plus `--config` overlays on argv, the disabled launch registering nothing, verbatim rendering, the split-turn merge and its character-based gate, the audit/stats mapping, the `.env` key read, the `Set`-based `<files>` block, the `FM_JEV_ENDPOINT` override, and full runs of the real vendored `compact()` and of the registered handler against a fake `fetch` - never contacting the real TypeSafe endpoint.
`fm-jev-compaction-live-e2e.test.sh` is the opt-in real-omp proof described under "Verified live against omp 18.2.8": six real omp RPC sessions over synthetic resumed histories against a local fake endpoint, asserting the full-size install, the honest text fallback, the three protection declines with zero outbound requests, and the worker-overlay launch.
