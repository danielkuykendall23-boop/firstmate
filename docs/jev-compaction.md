...
So before anything is sent, the handler reads `secrets.enabled` from the `config.yml` files omp itself layers - the agent directory's (`PI_CODING_AGENT_DIR`, else `~/.omp/agent`, `config.yml` or `config.yaml`) and the project's `.omp/config.yml`, the later file overriding the earlier as in omp - and declines when the switch is on, or when a file exists that it cannot read or spells the switch in a form it does not follow, so native compaction (which omp redacts) runs instead.
This covers manual `/compact` and automatic compaction alike, since omp emits the identical event from both.
A `--config` overlay or a `--config-override` flag is not visible to the extension, so a captain who enables Hide Secrets only that way must not enable this extension; enabling it in `config.yml` (the path `omp config set secrets.enabled true` writes) is what the gate reads.

Automatic compaction re-emits `session_before_compact` on each native method it tries in turn, so when the handler runs Jev and then declines (for example at the native cap), the identical region can be sent to Jev and declined again for every remaining native method before one succeeds; no result is wrong, but the round trips and the disclosure repeat.
...
On any failure - omp's Hide Secrets being on or unreadable, missing key, network error, malformed Jev response, a previous compaction whose substance lives in method-native `preserveData` a text-only summary cannot carry, a whole-context reduction below 25%, or a replacement above omp's native summary cap - the extension returns `undefined`, omp's own `compaction.methodOrder` runs unmodified, and the fallback is visibly logged (see next section).
