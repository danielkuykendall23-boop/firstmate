...
- `SECONDMATE_LIVENESS: secondmate <id>: skipped: <reason>|respawn failed after <cause>: <reason>` - the session-start liveness sweep could not guarantee that the registered secondmate is running a real agent process.
  Investigate the reason because that secondmate is not guaranteed live, EXCEPT a `skipped: deliberately stopped (...)` reason, which means `bin/fm-control.sh <id> exit` stopped it on purpose (`secondmate-provisioning` skill's Recovery section) and needs no action.
