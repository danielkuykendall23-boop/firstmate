...
The `<code-root>/.omp` directory package a crew or scout launch names is the deliberate exception: its Jev extensions register once per session even when a worktree's own `.omp/extensions/` copies also load (`../../../docs/jev.md`).
...
Launch a primary with plain `omp` inside the home (`FM_OMP_HARNESS=omp omp` when starting from a Claude pane); `../../../bin/fm-session-start.sh` prints `OMP_WATCH_EXTENSION: not loaded` when the running session has not loaded both tracked primary extensions.
