...
On omp, `/calm` is a narrower presentation-only toggle: it shows the same boat with an observed run-state line during active runs and collapses tool rows, and hides nothing else.
...
- [docs/calm.md](docs/calm.md) - current `/calm` behavior on each harness that supports it and its supported presentation limits.
- [docs/jev-compaction.md](docs/jev-compaction.md) - the optional, off-by-default Jev-guided compaction for omp: setup, what leaves the machine once opted in, and when it falls back to native compaction.
