...
async function fixture(options: { worker?: boolean; disabled?: boolean; flags?: string[] } = {}) {
...
if [ -e "$FM_HOME/off" ]; then echo 'dispatch-resolve: off (no key)' >&2; exit 0; fi
cat "$FM_HOME/result"
...
`);
  ...
  process.argv = ["node", "fixture", ...(options.flags ?? [])];
  ...
  let model = models[0];
  let thinking: Parameters<API["setThinkingLevel"]>[0] = "low";
  let clamp = false;
  const handlers = new Map<string, Handler>();
  const commands = new Map<string, Parameters<API["registerCommand"]>[1]>();
  const notices: string[] = [];
  const ctx: Context = { cwd: home, mode: "rpc", models: {
  ...
  const api: API = {
    ...
    setModel: async selected => { model = selected; return true; },
...
test("workers, explicit disable, unrelated directories and child modes are inert", async () => {
...
test("off, non-clear, malformed and nonzero resolver results leave selection unchanged", async () => {
  const f = await fixture();
  ...
  assert.deepEqual((await f.log()).map(row => row.status), ["off", "ambiguous", "escalate", "error", "error", "error"]);
